...
use nipart::{
    ...
    LinuxBridgeStpOptions, NipartError, VlanProtocol,
};

use crate::linux_bridge_port_vlan::parse_port_vlan_conf;

pub(crate) fn np_bridge_to_nipart(
    np_iface: &nispor::Iface,
    base_iface: BaseInterface,
) -> Result<LinuxBridgeInterface, NipartError> {
    let mut br_iface = LinuxBridgeInterface::new();
    let mut br_conf = LinuxBridgeConfig::new();
    br_iface.base = base_iface;
    br_conf.options = Some(np_bridge_options_to_nipart(np_iface)?);
...
fn np_bridge_options_to_nipart(
    np_iface: &nispor::Iface,
) -> Result<LinuxBridgeOptions, NipartError> {
...
//   * hello_time
//   * max_age
fn devide_by_user_hz(v: u32) -> Result<u32, NipartError> {
...
) -> Result<LinuxBridgeStpOptions, NipartError> {
