...
use nipart::{
    BaseInterface, ErrorKind, MptcpAddressFlag, MptcpConfig, NipartError,
};
            ...
            if mptcp_addr.address.to_string().as_str() == ip_addr {
                if let Some(np_flags) = mptcp_addr.flags.as_ref() {
                    for np_flag in np_flags {
                        match np_mptcp_addr_flag_to_nipart(*np_flag) {
                            Ok(f) => flags.push(f),
                            Err(e) => {
                                log::warn!("{}", e);
                                continue;
...
pub(crate) fn get_iface_mptcp_conf(
    ...
    if has_mptcp_valid_ip_addr {
        let mut conf = MptcpConfig::default();
        conf.address_flags = Some(flags);
        Some(conf)
    } else {
        None
    }
...
fn np_mptcp_addr_flag_to_nipart(
    value: nispor::MptcpAddressFlag,
) -> Result<MptcpAddressFlag, NipartError> {
    match value {
        ...
        nispor::MptcpAddressFlag::Fullmesh => Ok(MptcpAddressFlag::Fullmesh),
        _ => Err(NipartError::new(
            ErrorKind::NotSupportedError,
            format!(
                "MPTCP address flag {value:?} is not supported by nipart yet"
            ),
        )),
    }
}
