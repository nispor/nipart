...
pub(crate) fn np_mac_vlan_to_nipart(
    np_iface: &nispor::Iface,
    base_iface: BaseInterface,
) -> MacVlanInterface {
    let vlan_conf = np_iface.mac_vlan.as_ref().map(|np_vlan_info| {
        let mut conf = MacVlanConfig::default();
        conf.mode = match &np_vlan_info.mode {
        ...
        conf.accept_all_mac =
            Some(np_vlan_info.flags & MACVLAN_FLAG_NOPROMISC == 0);
        conf.base_iface = np_vlan_info.base_iface.clone();
        conf
    });

    let mut ret = MacVlanInterface::default();
    ret.base = base_iface;
    ret.mac_vlan = vlan_conf;
    ret
}

// TODO: remove below duplicate code
pub(crate) fn np_mac_vtap_to_nipart(
    np_iface: &nispor::Iface,
    base_iface: BaseInterface,
) -> MacVtapInterface {
    let vtap_conf = np_iface.mac_vtap.as_ref().map(|np_vtap_info| {
        let mut conf = MacVtapConfig::default();
        conf.mode = match &np_vtap_info.mode {
        ...
        conf.accept_all_mac =
            Some(np_vtap_info.flags & MACVTAP_FLAG_NOPROMISC == 0);
        conf.base_iface = np_vtap_info.base_iface.clone();
        conf
    });

    let mut ret = MacVtapInterface::default();
    ret.base = base_iface;
    ret.mac_vtap = vtap_conf;
    ret
}
