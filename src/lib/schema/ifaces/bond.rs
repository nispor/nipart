...
/// be flatted and [BondConfig] stored as `bond` section. The yaml
...
impl BondInterface {
    ...
    fn check_overlap_queue_id(&self) -> Result<(), NipartError> {
        ...
        if let Some(ports_conf) =
            self.bond.as_ref().and_then(|b| b.ports.as_deref())
        {
...
impl NipartInterface for BondInterface {
    ...
    fn post_merge_iface_specific(
            ...
            && let Some(new_bond_conf) = new_state.bond.as_ref()
        {
            ...
            bond_conf.ports = new_bond_conf.ports.clone();
            if bond_conf.ports.is_none()
                && let Some(old_ports_conf) =
                    old_state.bond.as_ref().and_then(|b| b.ports.as_ref())
            {
                bond_conf.ports = Some(
                    old_ports_conf
                        .iter()
...
pub struct BondConfig {
    ...
    /// warning message.
    /// Please refer to kernel documentation for detail
    #[serde(skip_serializing_if = "Option::is_none")]
    pub options: Option<BondOptions>,
    // TODO(Gris): Need to support nmstate schema for `port: Vec<String>`.
    /// Deserialize and serialize from/to `ports`.
    #[serde(skip_serializing_if = "Option::is_none", alias = "ports-config")]
    pub ports: Option<Vec<BondPortConfig>>,
}
